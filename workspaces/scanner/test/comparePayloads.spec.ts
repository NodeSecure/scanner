...
const kPayload = JSON.parse(
  readFileSync(join(kFixturePath, "/payload.json"), "utf8")
);
...
describe("comparePayloads", () => {
  ...
  it("should detect scanner version diff", () => {
    ...
    assert.strictEqual(scannerVersion?.prev, "1.0.0");
    assert.strictEqual(scannerVersion.now, "1.0.1");
  });

  it("should detect vulnerability strategy version diff", () => {
    const { vulnerabilityStrategy } = compareTo("vulnerabilityStrategyChanged");

    assert.strictEqual(vulnerabilityStrategy?.prev, "npm");
