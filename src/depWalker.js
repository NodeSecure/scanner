...
const { version: packageVersion } = JSON.parse(
  readFileSync(new URL(path.join("..", "package.json"), import.meta.url))
);
...
export async function depWalker(manifest, options = {}, logger = new Logger()) {
...

  {
    ...
    for await (const currentDep of getRootDependencies(
      manifest,
      rootDepsOptions
    )) {
      const { name, version, dev } = currentDep;

      const current = currentDep.exportAsPlainObject(
        name === manifest.name ? 0 : void 0
      );
      ...
      if (proceedDependencyAnalysis) {
        ...
        if (
          fetchedMetadataPackages.has(name) ||
          !current.versions[version].existOnRemoteRegistry
        ) {
        ...
        promisesToWait.push(
          scanDirOrArchive(name, version, {
            ref: current.versions[version],
            location,
            tmpLocation:
              forceRootAnalysis && name === manifest.name ? null : tmpLocation,
  ...
  const { hydratePayloadDependencies, strategy } = await vuln.setStrategy(
    vulnerabilityStrategy
  );
  ...
  for (const [packageName, dependency] of dependencies) {
    ...
    for (const [version, integrity] of Object.entries(metadataIntegrities)) {
      ...
      const isEmptyPackage = dependencyVer.warnings.some(
        (warning) => warning.kind === "empty-package"
      );
      if (isEmptyPackage) {
        globalWarnings.push(
          `${packageName}@${version} only contain a package.json file!`
        );
      }

      if (
        !("integrity" in dependencyVer) ||
        dependencyVer.flags.includes("isGit")
      ) {
        continue;
      }
      ...
      if (dependencyVer.integrity !== integrity) {
        globalWarnings.push(
          `${packageName}@${version} manifest & tarball integrity doesn't match!`
        );
      }
    }
  ...
  try {
    const { warnings, flaggedAuthors } = await getDependenciesWarnings(
      dependencies
    );
